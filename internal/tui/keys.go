...
func DefaultKeyMap() KeyMap {
	return KeyMap{
		...
		Logs:        key.NewBinding(key.WithKeys("l"), key.WithHelp("l", "pod logs")),
		YAML:        key.NewBinding(key.WithKeys("y"), key.WithHelp("y", "yaml of resource")),
		Wrap:        key.NewBinding(key.WithKeys("w"), key.WithHelp("w", "toggle wrap (logs)")),
		Back:        key.NewBinding(key.WithKeys("esc", "backspace"), key.WithHelp("esc", "back")),
		PrevSnap:     key.NewBinding(key.WithKeys("left"), key.WithHelp("←", "prev snap")),
